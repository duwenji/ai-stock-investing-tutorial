...
| `company_profiles`        | 全ユーザー共有 | 銘柄コードを主キーに、日本語名（`data_api/stock_price_api.py::fetch_japanese_name`）と業種・事業概要（`data_api/stock_price_api.py::fetch_company_profile`）を1テーブルに統合。取得元ごとに `name_updated_at`/`profile_updated_at` を独立管理し、それぞれ別のTTL（当日/30日）で更新判定する。                                                                                                                                                                                                                                                          |
