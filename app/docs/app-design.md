...
> 旧`conditions`形式（indicator/operatorスキーマ＋`sort_by`/`order`）は本節のパイプライン機能導入に伴い廃止した。移行前に保存されていた戦略は`steps`形式（`FILTER_BY_FUNDAMENTALS`＋`SORT_BY`の2ステップ）へ変換済みで、対話・評価・保存済み戦略の読み込み・管理者タブでのJSON編集のいずれも現在は`steps`形式のみを扱う。`strategy_builder/conditions.py::apply_strategy_conditions`自体は`FILTER_BY_FUNDAMENTALSステップの内部実装として残っている（後述）。
