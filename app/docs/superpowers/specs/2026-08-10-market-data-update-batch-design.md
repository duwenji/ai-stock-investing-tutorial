...
差分取得は「最新日付より後」しか見ないため、`price_history` の**途中**の欠損（管理者CRUD画面の `st.data_editor`（`num_rows="dynamic"`）による行削除等）は、鮮度切れ時の再取得では自動修復されなくなる（従来は鮮度切れのたびに直近5年分を丸ごと再取得していたため、途中欠損があっても次回アクセス時に自動的に埋まっていた）。通常運用（バッチによる日次更新）ではこの状況は発生しない。管理者が意図的に削除した場合は、当該tickerの `price_history` を全件削除すれば（`latest_date` が無くなり）次回全量バックフィルが走るため、手動での復旧手段は残る。定期フルリシンクは追加せず、このトレードオフを容認する。
