...
例えば「RSI逆張り戦略で上位50銘柄を選び、配当利回り3%以上で絞る」といった別の要望は、
v1のレジストリ（BACKTEST_RANK＋FILTER_BY_FUNDAMENTALS）の組み合わせだけで、コード変更
なしに今すぐ対応できる。さらに将来登場しうる、v1のレジストリに無い種類の要望（例:
セクターローテーション分析との連携）にも、レジストリに新しい関数を追加するだけで
対応できる拡張性を狙う。
