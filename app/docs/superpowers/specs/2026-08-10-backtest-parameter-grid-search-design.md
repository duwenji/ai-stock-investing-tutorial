...
- `docs/05-portfolio-management/03-backtest-automation.md`の演習課題2
  （「短期/長期の移動平均期間を変えて複数パターンでバックテストし、結果が大きく変動するかどうかを確認してください」）は
  演習課題としてそのまま残す（教材コードは`app/`と独立した最小実装のため、演習として手を動かす価値は変わらない）。
  ただし同ファイルには既に「関連: 発展版の実装例（`app/`）では…」という引用ブロック（28〜32行目、Prompt Chainingへの言及）の
  パターンがあるため、これに倣い演習課題2の直後に次の一文を追記する:
  > 関連: 発展版の実装例（`app/`）では、この「パラメータを変えて結果の変動を確認する」作業を
  > 近傍グリッドサーチと安定性判定（変動係数）として自動化しています。詳細は
  > [app/docs/app-design.md の4.3節](../../app/docs/app-design.md#43-バックテスト単一銘柄)を参照してください。
  > （このリンクは`docs/05-portfolio-management/03-backtest-automation.md`から見た相対パスであることに注意。）
