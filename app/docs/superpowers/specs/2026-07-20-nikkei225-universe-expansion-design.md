...
本設計では `UNIVERSE` を日経225の全構成銘柄（225銘柄）に差し替え、業種分布を広げるとともに、既存3タブの対象範囲そのものを拡大する。

## スコープ

- v1で実装する:
  - `screening/universe.py` の `UNIVERSE` / `UNIVERSE_NAMES` を日経225の225銘柄に差し替え
...
- ファイル冒頭に以下の趣旨のコメントを1行加える:
  > 日経225構成銘柄は実装時点（2026年7月）のものであり、日本経済新聞社による定期見直し・臨時入れ替えで変動する。定期的に公式発表と照合すること。

### 変更対象

```python
UNIVERSE: list[str] = [...]          # 60銘柄 → 225銘柄
UNIVERSE_NAMES: dict[str, str] = {...}  # 60銘柄分 → 225銘柄分
```
...
現状 `data_api/stock_price_api.py` の `fetch_universe_fundamentals` は対象銘柄を `for` ループで逐次 `fetch_fundamentals` 呼び出ししている。225銘柄になると初回実行（当日キャッシュなし時）が数分規模になりうるため、一括バックテスト（`app.py` 内 `map_concurrently` 使用箇所）と同じ並列化パターンを適用する。
...
銘柄選択肢（セレクトボックス）の候補が225件に増えるのみ。バックテストロジック・キャッシュキー構造は変更不要。

### 3. 一括バックテスト

`target_tickers = UNIVERSE ∪ 保有銘柄` が225件規模に拡大する。この処理は既に `map_concurrently` で並列取得済み（`app.py:520`）のためコード変更は不要。初回のキャッシュ生成時間が伸びる点は許容事項として扱う（当日2回目以降はキャッシュヒットで即時表示）。
...
- `tests/test_universe.py`:
  - `UNIVERSE` の件数が225件であること
  - `UNIVERSE` に重複がないこと
  - `UNIVERSE` の全ティッカーが `UNIVERSE_NAMES` のキーに存在すること（既存テストがあれば件数のみ更新、なければ新規追加）
...
- UI動作（初回スクリーニング実行時間・225件の選択肢表示）は `uv run python -m streamlit run app.py` で手動確認
