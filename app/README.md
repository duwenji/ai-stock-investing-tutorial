...
- **セクターローテーション**タブ: UNIVERSE銘柄を東証17業種に分類し、業種間の値動きの時差相関（リード・ラグ）を過去の株価データから計算します。相関ヒートマップとリード・ラグ上位ペアの表を表示し、上位5ペアにはAIによる解説コメントを付けます。あくまで過去の統計的傾向であり、将来の値動きを保証するものではありません。
...
sector_analysis/            # 業種別リターン・時差相関（リード・ラグ）計算
